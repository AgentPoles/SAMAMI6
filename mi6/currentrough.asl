...
+actionID(X):true <-
    ...
    if (map_initialized(true) & .empty(Path)) {
        if (has_target_dispenser(TX,TY,Type)) {
            ...
            ?current_pos(CX,CY);
            if (CX == TX & CY == TY) {  // First verify we're at target position
...
// Handle failed moves
+lastActionResult(failed_path)[source(percept)] : phase(moving) <-
    .my_name(Name);
    ?current_pos(CX,CY);
    ?failed_moves(F);
    if (F < 3) {  // Try waiting up to 3 times
        // Increment failed moves counter
        !update_failed_moves;
        .print("Agent ", Name, " move blocked at ", CX, ",", CY, " waiting (attempt ", F+1, ")");
    } else {
        // Too many failures, need to replan
        .print("Agent ", Name, " replanning after multiple blocked moves");
        -+failed_moves(0);  // Reset counter
        !switch_to_planning_mode; // Switch to planning phase
        -current_path(_);   // Clear current path
        +current_path([]);
        -has_target_dispenser(_,_,_);  // Clear target dispenser
        !plan_next_move;
    }.

// Combined handler for successful moves
+lastActionResult(success)[source(percept)] : true <-  // Reset failed moves counter
    .my_name(Name);
    .print("Agent ", Name, " successful move");
    ?current_pos(CX,CY);
    if (lastActionParams(Dir)) {
        !track_agent_position(Dir);
    }.

@track_agent_position[atomic]
+!track_agent_position(Dir) : true <- 
-+failed_moves(0);
...
@update_failed_moves[atomic]
+!update_failed_moves : true <-
  -+failed_moves(F+1).

@update_current_path[atomic]
+!update_current_path(path) : true <-
  -+current_path(path).
