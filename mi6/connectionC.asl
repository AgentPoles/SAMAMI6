...
+!find_unexplored_direction : true <-
    ...
    !generate_exploration_path(CX, CY, KnownCells, Path);
    if (Path == []) {
        .print("No unexplored directions found, returning to planning");
        -+phase(planning);
        -+exploring(false);
        -+exploration_steps(0);
        !start_planning_phase;
